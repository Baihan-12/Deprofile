...
class TimelineAgent:
    def __init__(self, profile, candidate_index, timeline_type = "life_event", timestamp = None):
        ...
        self.timeline_dir = str(DATA_DIR / f"stmhd_{self.timeline_type}_timeline")
    ...
    def _get_tweet_user_id(self):
        ...
        if self.candidate_index is None or self.candidate_index >= len(self.profile['candidate_id']):
            self.candidate_index = 0 # default to the first candidate
    ...
    def get_cut_timeline(self, timestamp = None, max_events_num = 50):
        ...
        cut_timeline = []
        current_index = -1
        while -current_index < len(self.timeline) and current_index < max_events_num and self.timeline[current_index]["timestamp"] > timestamp-90:
            cut_timeline.append(self.timeline[current_index])
            current_index -= 1
        return " ".join([f"{timestamp - event['timestamp']} days ago: {event[self.timeline_type]}-{event['tweet']}" for event in cut_timeline])
